...
res.mat <- data.frame(y = x.vals, 
                      density = dbinom(x.vals, size = n, prob = p),
...
for(i in 1:nrow(res.mat)){
  ...
  res.mat$cost.os[i] <- tmp.os
  res.mat$cost.us[i] <- tmp.us
  res.mat$total.cost[i] <- tmp.os*(cu+co) + cu*(n*p - tmp.p  )  #tmp.us*cu + tmp.os*co
}

kable(round(res.mat,2) , "pandoc")
#datatable(round(res.mat,2))
```
