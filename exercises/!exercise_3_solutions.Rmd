...
datatable(res.tab  , options = list(pageLength = nrow(res.tab) )) %>%
    formatRound(columns=c( "squ.LAR.2ES", "squ.LAR.3ES"), digits=3) 
...
Thus, at the end of 2007, for the first 7 months the following expected demands can be forecasted:
```{r, exe-3-2, echo = F}
...
Here, we have to calculate the expected demand under the condition that inter-arrival time is at least 3. The conditional inter-arrival time distribution is then
...
datatable(data.frame(month =  month.abb[4:8], forecast = ex.dem.sub))
```
