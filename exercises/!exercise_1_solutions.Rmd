...
Mean and median differ as the median is less sensitive to outliers. HOwevre, in expectation both estimators deliver identical measures of location (provided the forecasts are unbiased). 
...
Reasonable thresholds might be 100 and 750 for MSE as well as 0.005 and 0.1 for LAR. 
...
tab <- data.frame("ord.material.id(MSE)" = as.character(id.vec[id.ord.mse]), "MSE" = mse.ord, "class.mse" = class.mse, "ord.material.id(MSLA)" = as.character(id.vec[id.ord.msla]) , "mean.squared.LAR" = msla.ord, "class.msla" = class.msla )
...
(@) Use the material value information and a reasonable thresholds for ESFAR to categorize materials jointly in the ABC/RSU classes. Which recommendations for material provisioning strategies would you deduce?
...
tab <- data.frame("LAR" = msla.ord, "class.RSU" = class.msla , "cum.rel.val.share" = rel.cum.val.vec.ord[id.ord.msla] , "class.ABC" = class.vec[id.ord.msla] )

kable(tab, digits = c(3,0,2,0), "pandoc")
plot(msla.ord , rel.cum.val.vec.ord[id.ord.msla] , xlab=expression(Q[i]), pch=16, ylab="cum. rel. value share")
...
a.vec <- apply(cbind(s.vec, toval.vec),1 , max)
iqr.vec <- a.vec/s.vec
e.vec <- s.vec - a.vec
tab <- data.frame("material.id" = 1:n, "stock.value" = round(s.vec,1), "to.value" = round(toval.vec,1), "act.inv" = round(a.vec,1),  "iqr" = round(iqr.vec,1), "excess.inv" = round(e.vec,1))
kable(tab, digits = c(0,1,1,1,1,1),  caption = "Life-cycle analysis for materials 1-15", format = "pandoc")
names(iqr.vec) <- 1:n
plot(iqr.vec , rel.cum.val.vec.ord[as.character(1:n)] , xlab="IQR", pch=16, ylab="cum. rel. value share")
...
As all IQR values are greater than 1, for all materials higher stock levels should be realized. Particularly, A and B materials with IQR values $>10$ are critical.
