...
Additionally, $\kappa = \sum_{i=1}^n \rho_i=$ `r round(kappa,1)` is smaller than 1, too. Thus, a feasible shall exist.
...
kable(df,"pandoc", row.names = T) %>%
  ...
  add_header_above(c(" " = 1, "$m=1$" = 3, "$m=2$" = 3, "$m=4$" = 3, "$m=8$" = 3, "$m=16$" = 3))
```

Obviously, the schedule is feasible. The total cost are `r round(sum(c.cost.po2.3),2)`.
...
df <- data.frame(rbind( m.vec.int, round(m.vec.int*B, 2), round(m.vec.int*B*y.vec, 2) ))
colnames(df) <- reo.id 
...
Reoptimizing the basic cycle time yields $B=$ `r round(B.bt,2)` and the total cost are `r round(c.cost.bt,2)`.
